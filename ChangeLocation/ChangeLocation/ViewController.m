...
- (void)viewDidLoad {
    ...
    //保利大厦酒店 116.435397,39.93441
    //2.在进行坐标转换
    CLLocationCoordinate2D location2D = CLLocationCoordinate2DMake(116.356792, 40.075914);
    CLLocationCoordinate2D WGSlocation2D = [ChangeLoction gcj02ToWgs84:location2D];
    NSLog(@"转换===纬度：%f,经度：%f",WGSlocation2D.latitude , WGSlocation2D.longitude);
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray<CLLocation *> *)locations {
    ...
    [geoCoder reverseGeocodeLocation:currentLocation completionHandler:^(NSArray<CLPlacemark *> * _Nullable placemarks, NSError * _Nullable error) {
        if (placemarks.count >0) {
            ...
            //看需求定义一个全局变量来接收赋值
            NSString * resulut = [NSString stringWithFormat:@"当前位置 - %@ %@ %@ %@ %@",placeMark.country,currentCity,placeMark.subLocality,placeMark.thoroughfare,placeMark.name];
            NSString * resulut2 = [NSString stringWithFormat:@"具体海拔 - %f,%f",currentLocation.altitude,[currentLocation verticalAccuracy]];
            self.locationLable.text = [NSString stringWithFormat:@"%@/n,%@",resulut,resulut2];
        }
    }];
}
@end
